...
const buildDescription = (props: any): string => {
  ...
  const linkKeys = keys.filter((k) =>
    /^(hyperlink|url|website|link)$/i.test(k)
  );
  ...
  if (!desc) {
    const ignoreRegex =
      /^(name|title|description|desc|objectid|shape_len|shape_area|geometry|type)$/i;
    const parts: string[] = [];
    for (const k of keys) {
...
const extractObjectsFromGeometry = (
  geometry: any,
  props: any,
  layerId: string,
  objects: MapObject[]
  ...
  switch (geometry.type) {
    ...
    case "LineString":
      objects.push({
        ...
        title,
        description,
        positions: (geometry.coordinates || []).map((c: number[]) =>
          toLatLng(c)
        ),
      });
      break;
...
export const importFile = async (file: File): Promise<ImportResult> => {
  ...
  try {
    ...
    if (extension === "kmz") {
      ...
      const kmlFilename = Object.keys(unzipped.files).find((f) =>
      ...
      if (parserErrors.length === 0) {
        ...
        try {
          ...
          console.warn(
            "File extension was .kml but content was JSON. Parsed as GeoJSON."
          );
        } catch (e) {
      ...
      throw new Error(
        "Unsupported file format. Please upload .kml, .kmz, .json, or .geojson"
      );
    }
...
export const loadProjectFromFile = async (
  file: File
): Promise<{ layers: Layer[]; objects: MapObject[] }> => {
