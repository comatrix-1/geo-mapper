...
import { createMarkerIcon, createMoveHandleIcon, createVertexIcon } from './Icons';
...
const FlyToController = ({ flyToTarget, objects }: { flyToTarget: { id: string, time: number } | null | undefined, objects: MapObject[] }) => {
    const map = useMap();
...
const ObjectPopup = ({ obj, updateObjectMeta, onDelete, renderExtraView, renderExtraEdit }: ObjectPopupProps) => {
                ...
                {isEditing ? (
                    ...
                    <div className="flex flex-col gap-2">
                        <div>
                            ...
                            <input 
                                ...
                                value={obj.title} 
                                onChange={(e) => updateObjectMeta(obj.id, e.target.value, obj.description)}
                                placeholder="Title"
                            />
                        </div>
                        <div>
                            ...
                            <textarea
                                ...
                                onChange={(e) => updateObjectMeta(obj.id, obj.title, e.target.value)}
                    ...
                    <div className="flex flex-col gap-1">
                        <div className="flex flex-col">
                            <h3 className="font-bold text-lg text-gray-900 leading-tight">{obj.title || "Untitled"}</h3>
                            {obj.description && (
...
const RadiusInput = ({ value, onChange }: { value: number, onChange: (val: number) => void }) => {
    ...
    return (
        <div className="mt-1">
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Radius (m)</label>
            <input
                type="number"
                min="1"
...
const EditablePoint = ({ obj, style, isSelected, drawingMode, setSelectedObjectId, onUpdate, updateObjectMeta, onDelete }: MapObjectComponentProps) => {
    return (
        <Marker
            ...
            eventHandlers={{
                click: () => { if (drawingMode === 'none') setSelectedObjectId(obj.id); },
...
const EditableCircle = ({ obj, style, isSelected, drawingMode, setSelectedObjectId, onUpdate, updateObjectMeta, onDelete }: MapObjectComponentProps) => {
                    ...
                    renderExtraView={() => (
                        <div className="flex flex-col gap-1 text-sm">
                             <div className="flex justify-between">
                                ...
                                <span className="font-medium text-blue-700">{formatDistance(obj.radius)}</span>
                             </div>
...
const EditablePolyline = ({ obj, style, isSelected, drawingMode, setSelectedObjectId, onUpdate, updateObjectMeta, onDelete }: MapObjectComponentProps) => {
    ...
    return (
        <>
            <Polyline
                ...
                pathOptions={{ 
                    color: style.borderColor, 
                    weight: isSelected ? Math.max(style.borderWidth + 2, 6) : style.borderWidth
                }}
                interactive={drawingMode === 'none'}
                eventHandlers={{
                    click: () => { if (drawingMode === 'none') setSelectedObjectId(obj.id); }
                ...
                <ObjectPopup 
                    ...
                    renderExtraView={() => (
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium text-blue-700">{formatDistance(distance)}</span>
                        </div>
                    )}
                    renderExtraEdit={() => (
                        <div className="text-xs text-gray-400 italic">
            ...
            {isSelected && (
                <>
                    {showVertices && obj.positions.map((pos: LatLngLiteral, idx: number) => (
                        <Marker
                            key={`v-${idx}`}
                            position={pos}
...
const EditablePolygon = ({ obj, style, isSelected, drawingMode, setSelectedObjectId, onUpdate, updateObjectMeta, onDelete }: MapObjectComponentProps) => {
                ...
                <ObjectPopup 
                    ...
                    renderExtraView={() => (
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium text-blue-700">{formatArea(area)}</span>
                        </div>
                    )}
                    renderExtraEdit={() => (
                        <div className="text-xs text-gray-400 italic">
...
const MapEvents = ({ 
        ...
        if (drawingMode === 'point') {
            ...
            setDrawingMode('none'); 
             ...
             if (!circleCenter) {
                 ...
                 const radius = calculateDistance(circleCenter.lat, circleCenter.lng, point.lat, point.lng);
                 if (radius > 0) {
  ...
  return (
    <>
      ...
      {drawingMode === 'circle' && circleCenter && dragPoint && (() => {
          const radius = calculateDistance(circleCenter.lat, circleCenter.lng, dragPoint.lat, dragPoint.lng);
          return (
              <>
                  <Circle 
                    center={circleCenter} 
                  ...
                  <Polyline 
                    ...
                    pathOptions={{ color: 'blue', dashArray: '5, 5', weight: 1, opacity: 0.5 }} 
                    interactive={false}
                  />
                  <Marker position={dragPoint} icon={createVertexIcon()} opacity={0} interactive={false}>
      ...
      {(drawingMode === 'polyline' || drawingMode === 'polygon') && tempPoints.length > 0 && (
          <>
            {tempPoints.map((p, i) => (
                <Marker key={i} position={p} icon={createVertexIcon()} />
            ))}
            <Polyline positions={tempPoints} pathOptions={{ color: 'blue', dashArray: '5, 5' }} />
          </>
      )}
    </>
  );
        ...
        {rulerPoints.length > 0 && (
            <>
                ...
                {rulerPoints.length > 1 && (
                     <Popup position={rulerPoints[rulerPoints.length - 1]} offset={[0, -10]}>
